...
/*
 * Returns true if the given IOCTL command is safe to do,
 * even when it is invoked on a character or block device file.
 */
static int is_safe_ioctl_on_devices(unsigned int cmd)
{
			   ...
			   unsigned long arg)
{
	...
	if (!is_device || is_safe_ioctl_on_devices(cmd))
		return 0;
